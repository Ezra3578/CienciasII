...
public class MatrizAdyacencia implements RepresentacionGrafo, Cloneable {
    ...
    // necesario para algoritmos que modificarán los valores internos del grafo, como los de flujo maximo
    // evitando dañar el original
    public MatrizAdyacencia clone() {
        try {
            MatrizAdyacencia clone = (MatrizAdyacencia) super.clone();
 
            // Copia profunda del ArrayList de nodos (no comparte la lista original)
            clone.matriz = new ArrayList<>(this.matriz);
 
            // Copia profunda del HashMap externo Y de cada HashMap interno,
            // para que ninguna arista quede compartida con el grafo original
            HashMap<String, HashMap<String, Integer>> nodosCopia = new HashMap<>();
            for (Map.Entry<String, HashMap<String, Integer>> entrada : this.nodos.entrySet()) {
                nodosCopia.put(entrada.getKey(), new HashMap<>(entrada.getValue()));
            }
            clone.nodos = nodosCopia;
